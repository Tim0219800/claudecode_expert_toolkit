...
# Theme: detailed | Colors: true | Features: directory, git, model, context, usage, tokens, burnrate, weekly stats
# Enhanced with session and weekly statistics tracking
...
get_week_start() {
    if [[ "$OSTYPE" == "darwin"* ]]; then
        date -v-$(date +%u)d +%Y-%m-%d 2>/dev/null || date +%Y-%m-%d
    else
...
update_weekly_stats() {
    local session_cost="${1:-0}"
    local session_tokens="${2:-0}"
    local session_duration="${3:-0}"
    ...
    # Load existing weekly stats if same week
    if [ -f "$WEEKLY_STATS_FILE" ]; then
        local saved_week=$(grep -o '"week_start":"[^"]*"' "$WEEKLY_STATS_FILE" 2>/dev/null | cut -d'"' -f4)
        if [ "$saved_week" = "$week_start" ]; then
            total_cost=$(grep -o '"total_cost":[0-9.]*' "$WEEKLY_STATS_FILE" 2>/dev/null | cut -d':' -f2)
            total_tokens=$(grep -o '"total_tokens":[0-9]*' "$WEEKLY_STATS_FILE" 2>/dev/null | cut -d':' -f2)
            total_duration=$(grep -o '"total_duration_ms":[0-9]*' "$WEEKLY_STATS_FILE" 2>/dev/null | cut -d':' -f2)
            sessions=$(grep -o '"sessions":[0-9]*' "$WEEKLY_STATS_FILE" 2>/dev/null | cut -d':' -f2)
        fi
    fi

    # Session deduplication using project+duration as ID
    local session_id="${project_dir}_${session_duration}"
    local last_session_id=""
    [ -f "$SESSION_ID_FILE" ] && last_session_id=$(cat "$SESSION_ID_FILE" 2>/dev/null)

    if [ "$session_id" != "$last_session_id" ]; then
        total_cost=$(echo "${total_cost:-0} + ${session_cost:-0}" | bc 2>/dev/null || echo "${total_cost:-0}")
        total_tokens=$((${total_tokens:-0} + ${session_tokens:-0}))
        total_duration=$((${total_duration:-0} + ${session_duration:-0}))
...
EOF
    ...
    # Return stats (pipe-separated)
    echo "${total_cost:-0}|${total_tokens:-0}|${total_duration:-0}|${sessions:-0}"
}
...
# Line 5: Weekly statistics
week_color() { if [ "$use_color" -eq 1 ]; then printf '\033[38;5;213m'; fi; }  # pink/magenta

# Update and get weekly stats
weekly_data=$(update_weekly_stats "${cost_usd:-0}" "${tot_tokens:-0}" "${total_duration_ms:-0}" "${current_dir:-unknown}")
IFS='|' read -r weekly_cost weekly_tokens weekly_duration weekly_sessions <<< "$weekly_data"

# Format weekly duration
...
# Format weekly tokens with comma separator
weekly_tokens_fmt=$(printf "%'d" "${weekly_tokens:-0}" 2>/dev/null || echo "${weekly_tokens:-0}")

line5="📅 $(week_color)This week:$(rst) ${weekly_sessions:-0} sessions  💵 $(cost_color)\$${weekly_cost:-0}$(rst)  🕐 ${weekly_time_str}  📈 $(usage_color)${weekly_tokens_fmt} tok$(rst)"
